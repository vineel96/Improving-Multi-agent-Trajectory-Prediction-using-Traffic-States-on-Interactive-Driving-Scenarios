...
class MultimodalGenerativeCVAE(object):
    def __init__(self,
        ...
        self.state_length = int(np.sum([len(entity_dims) for entity_dims in self.state[node_type].values()])) - 1 # removed state value
    ...
    def create_node_models(self):
        ...
        if self.hyperparams['p_z_x_MLP_dims'] is not None:
            self.add_submodule(self.node_type + '/p_z_x',
                               model_if_absent=nn.Linear(x_size + 3 , self.hyperparams['p_z_x_MLP_dims'])) # added 3 for traffic state
        ...
        hxy_size = hxy_size + 3 # added 3 for one hot encoding feature size of traffic state
        ...
        self.add_submodule(self.node_type + '/decoder/rnn_cell',
                           model_if_absent=nn.GRUCell(decoder_input_dims + 3  , self.hyperparams['dec_rnn_dim'])) # added 3 for traffic state
        self.add_submodule(self.node_type + '/decoder/initial_h',
                           model_if_absent=nn.Linear(z_size + x_size + 3 , self.hyperparams['dec_rnn_dim'])) # added 3 for traffic state
    ...
    def create_edge_models(self, edge_types):
        for edge_type in edge_types:
            neighbor_state_length = int(
                np.sum([len(entity_dims) for entity_dims in self.state[edge_type.split('->')[1]].values()])) - 1 # added remove state variable
    ...
    def obtain_encoded_tensors(self,
        ...
        ### added ###
        traffic_state = np.zeros((batch_size, 3))
        for i_ in range(batch_size):
            if inputs[i_, -1, -1] == 0 :    #clump
                traffic_state[i_, :] = np.array([1, 0, 0])
            elif inputs[i_, -1, -1] == 1 :  #unclump
                traffic_state[i_, :] = np.array([0, 1, 0])
            elif inputs[i_, -1, -1] == 2 :  #neutral
                traffic_state[i_, :] = np.array([0, 0, 1])
        traffic_state = torch.from_numpy(traffic_state).type(torch.float).cuda()

        inputs = inputs[:, :, :6]
        inputs_st = inputs_st[:, :, :6]
        ### added ###
        ...
        ### added ###
        x_concat_list.append(traffic_state)
        ### added ###
    ...
    def encode_edge(self,
        ...
        for i, neighbor_states in enumerate(neighbors):  # Get neighbors for timestep in batch
            if len(neighbor_states) == 0:  # There are no neighbors for edge type # TODO necessary?
                ...
                ) - 1   # added to remove state variable
