        ...
        for batch in loader:
            ...
            for index, seq in enumerate(seq_start_end):
                ...
                for idx in range(num_of_peds_in_seq): # assign markers colors seperately for each agent in scene
                    ...
                    __ = ax.plot(obs_track_data[:, idx, 0], obs_track_data[:, idx, 1], 'x', color=colors[idx], alpha=1, label='History')
                    ...
                    for i_ in range(0,20):
                        if i_<=18:
                            ax.fill_between(prediction_agent[i_, :, 0], prediction_agent[i_, :, 1], prediction_agent[i_ + 1, :, 1], color=colors[idx],
                                        alpha=0.2)


                    '''pred_points =  np.concatenate(prediction_agent,axis=0)
                    ...
                    #for i_ in range(20):
                    #ax.fill(prediction_agent[:, :, 0], prediction_agent[:, :, 1], color=colors[idx], alpha=0.1 )


                #ax.legend()
                #plt.show()
                plt.savefig('./plots/with_pool_distribution/plot_' + str(count) + '.png', bbox_inches='tight')
                plt.close()
