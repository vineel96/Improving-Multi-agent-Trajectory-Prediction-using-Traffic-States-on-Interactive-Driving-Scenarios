...
class TrajectoryGenerator(nn.Module):
    def __init__(
        ...
        if self.mlp_decoder_needed():
            mlp_decoder_context_dims = [
                input_dim + 3   , mlp_dim, decoder_h_dim - self.noise_first_dim  #added
            ]
    ...
    def add_noise(self, _input, seq_start_end, user_noise=None):
        ...
        if self.noise_mix_type == 'global':  # global means all peds in a sequence gets same latent vector
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, obs_traffic_state, pred_traffic_state, user_noise=None): #added
        """
        Inputs:
        - obs_traj: Tensor of shape (obs_len, batch, 2)
        ...
        ## added ##
        traffic_state = np.zeros((batch,3))
        for i_ in range(batch):
            if obs_traffic_state[-1, i_, 0] == 0: #clump
                traffic_state[i_, :] = np.array([1,0,0])
            elif obs_traffic_state[-1, i_, 0] == 1: #unclump
                traffic_state[i_, :] = np.array([0,1,0])
            elif obs_traffic_state[-1, i_, 0] == 2: #neutral
        ...
        if self.pooling_type:
            ...
            mlp_decoder_context_input = torch.cat(
                [final_encoder_h.view(-1, self.encoder_h_dim), pool_h, traffic_state], dim=1) #added
        else:
            mlp_decoder_context_input = torch.cat( [final_encoder_h.view(
                -1, self.encoder_h_dim), traffic_state], dim= 1)  #added
