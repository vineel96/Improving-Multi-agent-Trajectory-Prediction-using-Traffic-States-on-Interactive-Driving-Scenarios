...
if __name__=="__main__":
    full_data_path = '../recorded_trackfiles/DR_CHN_Roundabout_LN_val/DR_CHN_Roundabout_LN_val.csv'
    ...
    for case_id in pd.unique(data['case_id']):
        ...
        #clump check
        l_c = d[ (d['x'] >= 924) & (d['x'] <= 924 + 14) & (d['y'] >= 1000) & (d['y'] <= 1000 + 7)] # left junction
        #t_c = d[ (d['x'] >= 1006) & (d['x'] <= 1006 + 11 ) & (d['y'] >= 1024) & (d['y'] <= 1024 + 14) ] # top   junction
        r_c = d[ (d['x'] >= 1054) & (d['x'] <= 1054 + 15 ) & (d['y'] >= 994) & (d['y'] <= 994 + 8) ] # right   junction
        #b_c = d[ (d['x'] >= 1027) & (d['x'] <= 1027 + 11 ) & (d['y'] >= 963) & (d['y'] <= 963 + 16) ] # bottom   junction
        # unclump check
        l_u = d[ (d['x'] >= 963) & (d['x'] <= 963 + 14) & (d['y'] >= 995) & (d['y'] <= 995 + 13)] # left junction
        #t_u = d[ (d['x'] >= 1001) & (d['x'] <= 1001 + 21) & (d['y'] >= 1004) & (d['y'] <= 1004 + 10) ] # top   junction
        r_u = d[ (d['x'] >= 1026) & (d['x'] <= 1026 + 13) & (d['y'] >= 994) & (d['y'] <= 994 + 14)] # right   junction
        #b_u = d[ (d['x'] >= 1019) & (d['x'] <= 1019 + 21) & (d['y'] >= 991) & (d['y'] <= 991 + 15) ] # bottom   junction
        # neutral check
        l_n = d[ (d['x'] >= 939) & (d['x'] <= 939 + 24) & (d['y'] >= 998) & (d['y'] <= 998 + 8) ] # left junction
        #t_n = d[ (d['x'] >= 1004) & (d['x'] <= 1004 + 15) & (d['y'] >= 1011) & (d['y'] <= 1011 + 13) ] # top   junction
        r_n = d[ (d['x'] >= 1038) & (d['x'] <= 1038 + 17) & (d['y'] >= 996) & (d['y'] <= 996 + 8) ] # right   junction
        ...
        if  len(l_c) > 0 and len(pd.unique(l_c['track_id'])) > 1 and len(np.where(l_c_check == True)[0]) >=2:
            l_c.to_csv('../interaction_dataset_state/{}/Roundabout_LN_left_clump_{}.txt'.format(mode, str(case_id)),header=None,index=None, sep=',')
            l_c_c+=1
        ...
        if  len(r_c) > 0 and len(pd.unique(r_c['track_id'])) > 1 and len(np.where(r_c_check == True)[0]) >=2:
            r_c.to_csv('../interaction_dataset_state/{}/Roundabout_LN_right_clump_{}.txt'.format(mode, str(case_id)),header=None,index=None, sep=',')
            r_c_c+=1
        ...
        if len(l_u) > 0 and len(pd.unique(l_u['track_id'])) > 1 and len(np.where(l_u_check == True)[0]) >=2:
            l_u.to_csv('../interaction_dataset_state/{}/Roundabout_LN_left_unclump_{}.txt'.format(mode, str(case_id)),
        ...
        if len(r_u) > 0 and len(pd.unique(r_u['track_id'])) > 1 and len(np.where(r_u_check == True)[0]) >=2:
            r_u.to_csv(
                '../interaction_dataset_state/{}/Roundabout_LN_right_unclump_{}.txt'.format(mode, str(case_id)),
                header=None, index=None, sep=',')
            r_u_c+=1
        ...
        if len(l_n) > 0 and len(pd.unique(l_n['track_id'])) > 1 and len(np.where(l_n_check == True)[0]) >=2:
            l_n.to_csv('../interaction_dataset_state/{}/Roundabout_LN_left_neutral_{}.txt'.format(mode, str(case_id)),
                       header=None, index=None, sep=',')
            l_n_c+=1
        ...
        if len(r_n) > 0 and len(pd.unique(r_n['track_id'])) > 1 and len(np.where(r_n_check == True)[0]) >=2:
            r_n.to_csv(
                '../interaction_dataset_state/{}/Roundabout_LN_right_neutral_{}.txt'.format(mode, str(case_id)),
                header=None, index=None, sep=',')
            r_n_c+=1
